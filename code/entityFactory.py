    ...
    def get_entity(entity_name: str, position=(0, 0)):  # 0,0 é o padrão
        match entity_name:
            case 'level1_bg':  # Nome dos bg. Carregue todos em uma lista
                list_bg = []
                for i in range(
                        4):  # Aqui, optei por tirar a última camada (que tem uns matinhos e umas pedras) pra ficar mais bonitinho. Tava muito feio com essa última.
                    list_bg.append(Background(f'lvl1_bg{i}', (0, 0)))
                    list_bg.append(Background(f'lvl1_bg{i}', (WIN_WIDTH,
                                                              0)))  # Precisamos fazer isso para ir repetindo as imagens conforme as anteriores saem da tela
            ...
            case 'level2_bg':  # Nome dos bg. Carregue todos em uma lista
                list_bg = []
                for i in range(4):  # Aqui, deixei todas. Mudei de ideia
                    list_bg.append(Background(f'lvl2_bg{i}', (0, 0)))
                    list_bg.append(Background(f'lvl2_bg{i}', (WIN_WIDTH,
                                                              0)))  # Precisamos fazer isso para ir repetindo as imagens conforme as anteriores saem da tela
                    # Agora precisamos fazer essas 10 imagens rodarem infinitamente. Veja no background.py como é feito.
                return list_bg
            case 'Player1':
            ...
            case 'Enemy1':
                return Enemy('Enemy1', (WIN_WIDTH + 10, random.randint(200, 485)))
            case 'Enemy2':
                return Enemy('Enemy2', (WIN_WIDTH + 10, random.randint(200, 485)))
